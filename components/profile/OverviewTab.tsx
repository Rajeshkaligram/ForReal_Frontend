...
export default function OverviewTab({ setTab }: { setTab: (t: string) => void }) {
  ...
  const getRentalImage = (item: RentedProduct) => {
    ...
    if (imgs.length > 0) {
      const transformed = transformImageUrl(imgs[0]);
      return transformed || "/assets/images/p1.jpeg";
    }
    return transformImageUrl(item.product?.picture) || "/assets/images/p1.jpeg";
  };
            ...
            {loadingSaved ? (
              ...
              <div className="grid grid-cols-2 gap-3">
                {savedPieces.map((item) => {
                  ...
                  const imgSrc =
                    transformImageUrl(product?.picture || product?.thumbnail) ||
                    "/assets/images/p1.jpeg";
                  return (
                    <Link
                      key={item.id}
