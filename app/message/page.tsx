...
function MessageContent() {
  ...
  const fetchInbox = async () => {
    try {
      ...
      // Handle both new API structure (data.messages) and legacy (data as array)
      let allMsgs: Message[] = [];
      if (res?.data) {
        if (Array.isArray(res.data)) {
          allMsgs = res.data;
        } else if ('messages' in res.data) {
          allMsgs = res.data.messages;
        }
      }
      ...
      if (activeRoomId) {
         ...
         const existingThread = allMsgs.find((m: Message) => 
  ...
  const fetchActiveChat = async (roomId: string | number) => {
    ...
    try {
      ...
      // Handle both API structures: data as array or data.messages
      let msgs: Message[] = [];
      if (res?.data) {
        if (Array.isArray(res.data)) {
          msgs = res.data;
      ...
      // Extract counterpart user ID from room messages
      // The counterpart is the user who is NOT the current logged-in user
      if (msgs.length > 0 && authUser?.id) {
        const firstMsg = msgs[0];
        const counterpartId = firstMsg.from_user_id === authUser.id
          ? firstMsg.to_user_id
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    // Debug: Log the receiver ID being sent
    console.log('Sending message to receiver_id:', receiverId);

    // Create a local mock message just in case the API is not yet implemented
    const mockMessage: Message = {
      id: Date.now(),
      from_user_id: authUser?.id || 0,
      to_user_id: Number(receiverId),
      content: newMessage.trim(),
      sent: "just now",
      first_name: authUser?.first_name || "You",
          ...
          <div className={`md:w-1/3 border-r border-border flex flex-col ${activeRoomId || initialUserId ? 'hidden md:flex' : 'flex'}`}>
            ...
            <div className="flex-1 overflow-y-auto">
              {loadingInbox ? (
                ...
                rooms.map(room => {
                  // Use user_id (new API) or sender_id/receiver_id (legacy) to determine counterpart
                  const isOwnMessage = room.sender_id === authUser?.id || room.user_id === authUser?.id;
                  const counterpartId = isOwnMessage
                    ? (room.receiver_id || 0)
                    : (room.user_id || room.sender_id || 0);
                  // Use new API fields (first_name, last_name) or fallback to sender object
                  const counterpartName = isOwnMessage
                    ? `Counterpart #${room.receiver_id || ''}`
                    : `${room.first_name || room.sender?.first_name || ''} ${room.last_name || room.sender?.last_name || ''}`.trim() || `User #${counterpartId}`;
                  const isActive = activeRoomId === room.room_id;
                  const displayMessage = room.content || room.message || '';
                  const displayTime = room.sent || (room.created_at ? formatTime(room.created_at) : '');
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-start mb-1">
                        ...
                        <p className="text-[10px] text-muted whitespace-nowrap">{displayTime}</p>
                      </div>
                      <p className="text-xs text-muted truncate">{displayMessage}</p>
                    </div>
                  )
                })
              )}
                ...
                <div className="flex-1 overflow-y-auto p-5 space-y-4 bg-[#FDFCF8]/50">
                  {loadingChat ? (
                    ...
                    activeChatMessages.map((msg) => {
                      const isOwn = msg.from_user_id === authUser?.id || msg.sender_id === authUser?.id;
                      const displayMessage = msg.content || msg.message || '';
                      const displayTime = msg.sent || (msg.created_at ? formatTime(msg.created_at) : '');
                      return (
                        <div key={msg.id} className={`flex ${isOwn ? "justify-end" : "justify-start"}`}>
                          <div className={`max-w-[80%] md:max-w-[65%] px-4 py-3 text-sm leading-relaxed ${isOwn ? "bg-black text-white" : "bg-white border border-border text-black shadow-sm"}`}>
                            <p>{displayMessage}</p>
                            {displayTime && (
