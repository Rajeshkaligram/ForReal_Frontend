  ...
  useEffect(() => {
    ...
    productsAPI
      ...
      .then((res) => {
        ...
        if (pd.picture) {
          const transformed = transformImageUrl(pd.picture);
          if (transformed) allPhotos.push(transformed);
        }
        if (pd.product_photos) {
          const transformedPhotos = pd.product_photos
            .map((p: any) => transformImageUrl(p.sub_photo))
            .filter(Boolean);
